  ...
  const handleDelete = async (id) => {
    try {
      ...
      if (response.data.Status) {
        ...
        alert('Failed to delete the record: ' + response.data.Error);
      }
    } catch (err) {
              ...
              <tr key={e._id} className="border-b">
                ...
                <td className="p-3 border">
                  <Link
                    ...
                    className="bg-green-600 text-white rounded-md px-3 py-1 mr-2 hover:bg-green-500 transition"
                  >
                    Edit
                  </Link>
                  <button
                    className="bg-red-600 text-white rounded-md px-3 py-1 hover:bg-red-500 transition"
